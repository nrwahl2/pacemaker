                       ...
                       gboolean prepend, const char *raised_from)
{
    ...
    last_data_id++;
    pcmk__trace("%s %s FSA input %llu (%s) due to %s, %s data",
    ...
    if (with_actions != A_NOTHING) {
        pcmk__trace("Adding actions %.16" PRIx64 " to input", with_actions);
    }
    ...
    if (data != NULL) {
        switch (cause) {
            ...
            case C_HA_MESSAGE:
                ...
                pcmk__trace("Copying %s data from %s as cluster message data",
    ...
    pcmk__trace("FSA message queue length is %u",
                controld_fsa_message_queue_length());
...
delete_fsa_input(fsa_data_t * fsa_data)
{
    ...
    pcmk__trace("About to free %s data", fsa_cause2string(fsa_data->fsa_cause));

    if (fsa_data->data != NULL) {
        delete_ha_msg_input(fsa_data->data);
        pcmk__trace("%s data freed", fsa_cause2string(fsa_data->fsa_cause));
    }
...
relay_message(xmlNode * msg, gboolean originated_locally)
{
    ...
    host_to = pcmk__xe_get(msg, PCMK__XA_CRM_HOST_TO);
    sys_to = pcmk__xe_get(msg, PCMK__XA_CRM_SYS_TO);
    sys_from = pcmk__xe_get(msg, PCMK__XA_CRM_SYS_FROM);
    type = pcmk__xe_get(msg, PCMK__XA_T);
    task = pcmk__xe_get(msg, PCMK__XA_CRM_TASK);
    ref = pcmk__xe_get(msg, PCMK_XA_REFERENCE);
    ...
    if (broadcast) {
        ...
        const char *mode = pcmk__xe_get(msg_data, PCMK__XA_MODE);
    ...
    if (is_for_dc && pcmk__str_eq(task, CRM_OP_PING, pcmk__str_casei) 
        ...
        sys_to = pcmk__xe_get(reply, PCMK__XA_CRM_SYS_TO);
    ...
    if (is_for_dc || is_for_dcib || is_for_te) {
        if (AM_I_DC) {
            ...
            pcmk__trace("Route message %s locally as DC request", ref);
    ...
    if (is_local) {
        if (is_for_crm || is_for_cib) {
            pcmk__trace("Route message %s locally as controller request", ref);
            return FALSE; // More to be done by caller
        }
                               ...
                               const char *proxy_session)
{
    ...
    const char *op = pcmk__xe_get(client_msg, PCMK__XA_CRM_TASK);
    const char *ref = pcmk__xe_get(client_msg, PCMK_XA_REFERENCE);
...
handle_message(xmlNode *msg, enum crmd_fsa_cause cause)
{
    ...
    type = pcmk__xe_get(msg, PCMK__XA_SUBT);
...
handle_failcount_op(xmlNode * stored_msg)
{
    ...
    if (pcmk__xe_get(xml_op, PCMK__XA_ROUTER_NODE)) {
        is_remote_node = TRUE;
    }
...
{
    const char *dc_version = pcmk__xe_get(msg, PCMK_XA_CRM_FEATURE_SET);
    ...
    if (feature_set_compatible(dc_version, CRM_FEATURE_SET)) {
        pcmk__trace("Local feature set (%s) is compatible with DC's (%s)",
...
handle_request(xmlNode *stored_msg, enum crmd_fsa_cause cause)
{
    ...
    if (strcmp(op, CRM_OP_SHUTDOWN_REQ) == 0) {
        const char *from = pcmk__xe_get(stored_msg, PCMK__XA_SRC);
