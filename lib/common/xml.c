    ...
    for (size_t index = 0; index < length; index++) {
        ...
        switch (text[index]) {
            ...
            case '>':
                return true;
            case '&':
                // @TODO Don't escape if it's the beginning of an entity ref
                return true;
            case '"':
                if (for_attr) {
                    return true;
 ...
 *   example, \c "&#x0A;" for \c '\n') does not get replaced. The processor then
 *   replaces any sequence of space (\c '#x20') characters with a single space
 *   character.
 *   * For more details, see the "Attribute-Value Normalization" section of the
 *     XML spec, currently section 3.3.3:
 *     https://www.w3.org/TR/xml/#AVNormalize
 *
 * Pacemaker always delimits attribute values with double quotes, so this
 * function doesn't escape single quotes.
 *
 * We sometimes need to preserve literal whitespace characters in attribute
 * values, so this function replaces them by their respective character
 * references. However, a processor will still replace a sequence of space
 * (\c '#x20') characters with a single one, unless the attribute type is CDATA;
 * using character references (\c "&#x20;") cannot prevent that.
 *
