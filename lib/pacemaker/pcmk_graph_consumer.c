...
pcmk__unpack_graph(const xmlNode *xml_graph, const char *reference)
{
    ...
    if (xml_graph != NULL) {
        ...
        buf = crm_element_value(xml_graph, PCMK__OPT_CLUSTER_DELAY);
        CRM_CHECK(buf != NULL, free(new_graph);
                               return NULL);
        new_graph->network_delay = crm_parse_interval_spec(buf);

        buf = crm_element_value(xml_graph, PCMK__OPT_STONITH_TIMEOUT);
        if (buf == NULL) {
        ...
        buf = crm_element_value(xml_graph, PCMK__OPT_MIGRATION_LIMIT);
