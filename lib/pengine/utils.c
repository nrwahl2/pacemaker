...
get_target_role(const pcmk_resource_t *rsc, enum rsc_role_e *role)
{
    ...
    const char *value = g_hash_table_lookup(rsc->meta, PCMK__META_TARGET_ROLE);

    CRM_CHECK(role != NULL, return FALSE);

    if (pcmk__str_eq(value, PCMK__ROLE_STARTED,
    ...
    if (local_role == pcmk_role_unknown) {
        pcmk__config_err("Ignoring '" PCMK__META_TARGET_ROLE "' for %s "
...
pe__resource_is_disabled(const pcmk_resource_t *rsc)
{
    ...
    target_role = g_hash_table_lookup(rsc->meta, PCMK__META_TARGET_ROLE);
    if (target_role) {
