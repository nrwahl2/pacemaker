...
{
    const char *remote_addr = g_hash_table_lookup(params, PCMK__REMOTE_RA_ADDR);

    if (pcmk__str_eq(remote_addr, "#uname", pcmk__str_none)) {
        GHashTable *base = pe_rsc_params(rsc, NULL, rsc->cluster);

        remote_addr = g_hash_table_lookup(base, PCMK__REMOTE_RA_ADDR);
        if (remote_addr != NULL) {
            g_hash_table_insert(params, strdup(PCMK__REMOTE_RA_ADDR),
                                strdup(remote_addr));
        }
    }
}
 ...
 * * The resource's \c PCMK__META_CONTAINER_ATTR_TARGET meta-attribute (usually
 *   set only for bundles), as \c PCMK__META_CONTAINER_ATTR_TARGET
