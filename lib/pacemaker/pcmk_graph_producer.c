...
add_resource_details(const pcmk_action_t *action, xmlNode *action_xml)
{
    ...
    /* If a resource is locked to a node via PCMK__OPT_SHUTDOWN_LOCK, mark its
     * actions so the controller can preserve the lock when the action
     * completes.
     */
    if (pcmk__action_locks_rsc_to_node(action)) {
        crm_xml_add_ll(action_xml, PCMK__OPT_SHUTDOWN_LOCK,
...
pcmk__create_graph(pcmk_scheduler_t *scheduler)
{
    ...
    value = pe_pref(scheduler->config_hash, PCMK__OPT_CLUSTER_DELAY);
    crm_xml_add(scheduler->graph, PCMK__OPT_CLUSTER_DELAY, value);

    value = pe_pref(scheduler->config_hash, PCMK__OPT_STONITH_TIMEOUT);
    crm_xml_add(scheduler->graph, PCMK__OPT_STONITH_TIMEOUT, value);
    ...
    value = pe_pref(scheduler->config_hash, PCMK__OPT_MIGRATION_LIMIT);
    if ((pcmk__scan_ll(value, &limit, 0LL) == pcmk_rc_ok) && (limit > 0)) {
        crm_xml_add(scheduler->graph, PCMK__OPT_MIGRATION_LIMIT, value);
    }
