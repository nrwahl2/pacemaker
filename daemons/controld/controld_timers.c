...
//! \c PCMK__OPT_JOIN_INTEGRATION_TIMEOUT
static fsa_timer_t *integration_timer = NULL;

//! \c PCMK__OPT_JOIN_FINALIZATION_TIMEOUT
...
controld_configure_fsa_timers(GHashTable *options)
{
    ...
    // Election timer
    value = g_hash_table_lookup(options, PCMK__OPT_DC_DEADTIME);
    pcmk__parse_interval_spec(value, &(election_timer->period_ms));

    // Integration timer
    value = g_hash_table_lookup(options, PCMK__OPT_JOIN_INTEGRATION_TIMEOUT);
    pcmk__parse_interval_spec(value, &(integration_timer->period_ms));

    // Finalization timer
    value = g_hash_table_lookup(options, PCMK__OPT_JOIN_FINALIZATION_TIMEOUT);
    pcmk__parse_interval_spec(value, &(finalization_timer->period_ms));

    // Shutdown escalation timer
    value = g_hash_table_lookup(options, PCMK__OPT_SHUTDOWN_ESCALATION);
    ...
    // Transition timer
    value = g_hash_table_lookup(options, PCMK__OPT_TRANSITION_DELAY);
    pcmk__parse_interval_spec(value, &(transition_timer->period_ms));

    // Recheck interval
    value = g_hash_table_lookup(options, PCMK__OPT_CLUSTER_RECHECK_INTERVAL);
