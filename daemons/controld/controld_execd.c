...
controld_rc2event(lrmd_event_data_t *event, int rc)
{
    ...
    switch (rc) {
        ...
        case EACCES:
            lrmd__set_result(event, PCMK_OCF_INSUFFICIENT_PRIV, PCMK_EXEC_ERROR,
                             NULL);
            break;
...
static bool
cancel_op_key(lrm_state_t *lrm_state, lrmd_rsc_info_t *rsc, const char *key,
              bool remove)
{
    ...
    pcmk__trace("Removed %u op cache entries, new size: %u", removed,
 ...
 */
static int
get_lrm_resource(lrm_state_t *lrm_state, const xmlNode *rsc_xml, bool do_create,
                 lrmd_rsc_info_t **rsc_info)
{
    ...
    pcmk__trace("Retrieving resource information for %s from the executor", id);
    ...
    if ((*rsc_info == NULL) && do_create) {
        ...
        if (rc != pcmk_rc_ok) {
            pcmk__err("Could not register resource %s with the executor on %s: "
                      "%s " QB_XS " rc=%d", id, lrm_state->node_name,
                      pcmk_rc_str(rc), rc);
                ...
                bool unregister, bool from_cib)
{
    ...
    if (rc == pcmk_rc_ok) {
        ...
        pcmk__warn("Could not delete '%s' from executor for %s%s%s: %s "
                   QB_XS " rc=%d", id, sys, ((user != NULL)? " as " : ""),
                   pcmk__s(user, ""), pcmk_rc_str(rc), rc);
    }
...
get_fake_call_id(lrm_state_t *lrm_state, const char *rsc_id)
{
     ...
     * otherwise the failure will not result in a possible recovery of the
     * resource as it could appear the failure occurred before the successful
     * start
     */
    if (entry != NULL) {
              ...
              bool is_remote_node, bool reprobe_all_nodes)
{
    ...
    while (g_hash_table_iter_next(&gIter, NULL, (void **) &entry)) {
        /* Only unregister the resource during a reprobe if it is not a remote
    ...
    controld_delete_node_history(lrm_state->node_name, false);
}
...
static bool
do_lrm_cancel(ha_msg_input_t *input, lrm_state_t *lrm_state,
...
{
    lrmd_rsc_info_t *rsc = NULL;
    xmlNode *xml_rsc = pcmk__xe_first_child(input->xml, PCMK_XE_PRIMITIVE, NULL,
                                            NULL);
    ...
    if ((rc != pcmk_rc_ok) && !create_rsc) {
        /* Delete of malformed or nonexistent resource (deleting something that
         * does not exist is a success)
         */
    ...
    if (rc == EINVAL) {
        ...
        pcmk__err("Invalid resource definition for %s", pcmk__xe_id(xml_rsc));
    ...
    if (rc != pcmk_rc_ok) {
        ...
        pcmk__err("Could not register resource '%s' with executor: %s "
                  QB_XS " rc=%d", pcmk__xe_id(xml_rsc), pcmk_rc_str(rc), rc);
...
verify_stopped(enum crmd_fsa_state cur_state, int log_level)
{
    ...
    for (GList *state_entry = lrm_state_list; state_entry != NULL;
                  ...
                  active_op_t *pending, const xmlNode *action_xml)
{
    ...
    /* If a shutdown was escalated while operations were pending, then the FSA
     * will be stalled right now... allow it to continue
     */
