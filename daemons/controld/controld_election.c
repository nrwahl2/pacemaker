...
{
    const char *value = g_hash_table_lookup(options,
                                            PCMK__OPT_ELECTION_TIMEOUT);
               ...
               enum crmd_fsa_input current_input, fsa_data_t * msg_data)
{
    ...
    dc_takeover_update_attr(PCMK__OPT_HAVE_WATCHDOG, pcmk__btoa(watchdog));
    dc_takeover_update_attr(PCMK__OPT_DC_VERSION,
    ...
    if ((controld_globals.cluster_name == NULL) && is_corosync_cluster()) {
        ...
        if (cluster_name != NULL) {
            dc_takeover_update_attr(PCMK__OPT_CLUSTER_NAME, cluster_name);
        }
        free(cluster_name);
    }
#endif
