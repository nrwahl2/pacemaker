...
pcmk__pid_active(pid_t pid, const char *expected_path)
{
    ...
    if ((expected_path == NULL) || !pcmk__procfs_has_pids()) {
    ...
    /* Make sure the PID hasn't been reused by another process.
     *
     * @FIXME The PID might still belong to a zombie, which could confuse
     * decisions.
     */
