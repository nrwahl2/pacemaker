 ...
 *
 * \param[in] preference  Cluster-wide \c PCMK__OPT_NODE_ACTION_LIMIT from the
 *                        CIB
 */
static void
...
{
    const char *value = g_hash_table_lookup(options, PCMK__OPT_LOAD_THRESHOLD);

    if (value != NULL) {
        throttle_set_load_target(strtof(value, NULL) / 100.0);
    }

    value = g_hash_table_lookup(options, PCMK__OPT_NODE_ACTION_LIMIT);
...
throttle_get_total_job_limit(int l)
{
    ...
    if(limit == l) {
        ...
        crm_trace("Using " PCMK__OPT_BATCH_LIMIT "=%d", limit);

    } else {
        crm_trace("Using " PCMK__OPT_BATCH_LIMIT "=%d instead of %d", limit, l);
    }
    return limit;
}
