...
detect_promotable(pcmk_resource_t *rsc)
{
    ...
    if (pcmk__xe_is(rsc->xml, PCMK_XE_PROMOTABLE_LEGACY)) {
         ...
         */
        g_hash_table_insert(rsc->meta, strdup(PCMK__META_PROMOTABLE),
...
unpack_requires(pcmk_resource_t *rsc, const char *value, bool is_default)
{
    if (pcmk__str_eq(value, PCMK__VALUE_NOTHING, pcmk__str_casei)) {
        ...
        if (pcmk_is_set(rsc->flags, pcmk_rsc_fence_device)) {
            pcmk__config_warn("Resetting \"" PCMK__META_REQUIRES "\" for %s "
        ...
        if (orig_value != NULL) {
            pcmk__config_err("Resetting '" PCMK__META_REQUIRES "' for %s "
                    ...
                    pcmk_resource_t *parent, pcmk_scheduler_t *scheduler)
{
    ...
    *rsc = calloc(1, sizeof(pcmk_resource_t));
    ...
    value = g_hash_table_lookup((*rsc)->meta, PCMK__META_PRIORITY);
    (*rsc)->priority = char2score(value);

    value = g_hash_table_lookup((*rsc)->meta, PCMK__META_CRITICAL);
    if ((value == NULL) || crm_is_true(value)) {
        pe__set_resource_flags(*rsc, pcmk_rsc_critical);
    }

    value = g_hash_table_lookup((*rsc)->meta, PCMK__META_NOTIFY);
    ...
    value = g_hash_table_lookup((*rsc)->meta, PCMK__META_ALLOW_MIGRATE);
    if (crm_is_true(value)) {
    ...
    value = g_hash_table_lookup((*rsc)->meta, PCMK__META_IS_MANAGED);
    if (value != NULL) {
        if (crm_is_true(value)) {
            pe__set_resource_flags(*rsc, pcmk_rsc_managed);
        } else {
    ...
    value = g_hash_table_lookup((*rsc)->meta, PCMK__META_MAINTENANCE);
    if (crm_is_true(value)) {
        pe__clear_resource_flags(*rsc, pcmk_rsc_managed);
    ...
    if (pe_rsc_is_clone(pe__const_top_resource(*rsc, false))) {
        value = g_hash_table_lookup((*rsc)->meta, PCMK__META_GLOBALLY_UNIQUE);
        if (crm_is_true(value)) {
    ...
    value = g_hash_table_lookup((*rsc)->meta, PCMK__META_MULTIPLE_ACTIVE);
    ...
    value = g_hash_table_lookup((*rsc)->meta, PCMK__META_RESOURCE_STICKINESS);
    if (value != NULL) {
    ...
    value = g_hash_table_lookup((*rsc)->meta, PCMK__META_REQUIRES);
    ...
    if (remote_node) {
         ...
         */
        value = g_hash_table_lookup(params, PCMK__REMOTE_RA_RECONNECT_INTERVAL);
        if (value) {
