               ...
               pcmk_scheduler_t *scheduler)
{
    if (pe_rsc_is_clone(rsc)) {
        ...
        pcmk__config_warn("Ignoring " PCMK__META_GLOBALLY_UNIQUE " for %s "
...
clone_unpack(pcmk_resource_t *rsc, pcmk_scheduler_t *scheduler)
{
    ...
    if (pcmk_is_set(rsc->flags, pcmk_rsc_promotable)) {
        ...
        clone_data->promoted_max =
            unpack_meta_int(rsc, PCMK__META_PROMOTED_MAX,
    ...
    if (crm_is_true(g_hash_table_lookup(rsc->meta, PCMK__META_ORDERED))) {
     ...
     */
    if (g_hash_table_lookup(rsc->meta,
                            PCMK__META_RESOURCE_STICKINESS) == NULL) {
        add_hash_param(rsc->meta, PCMK__META_RESOURCE_STICKINESS, "1");
    }
     ...
     */
    add_hash_param(rsc->meta, PCMK__META_GLOBALLY_UNIQUE,
                   pe__rsc_bool_str(rsc, pcmk_rsc_unique));
...
configured_role(pcmk_resource_t *rsc)
{
    ...
    if (target_role != NULL) {
        ...
        if (role == pcmk_role_unknown) {
            pcmk__config_err("Invalid " PCMK__META_TARGET_ROLE
