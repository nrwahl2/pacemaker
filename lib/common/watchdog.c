...
pcmk__valid_sbd_timeout(const char *value)
{
    ...
    if (st_timeout < 0) {
        ...
        crm_debug("Using calculated value %ld for "
                  PCMK__OPT_STONITH_WATCHDOG_TIMEOUT " (%s)",
                  st_timeout, value);
    }

    if (st_timeout == 0) {
        crm_debug("Watchdog may be enabled but "
                  PCMK__OPT_STONITH_WATCHDOG_TIMEOUT " is disabled (%s)",
                  value? value : "default");

    } else if (pcmk__locate_sbd() == 0) {
        crm_emerg("Shutting down: " PCMK__OPT_STONITH_WATCHDOG_TIMEOUT
        ...
        crm_info("Watchdog configured with " PCMK__OPT_STONITH_WATCHDOG_TIMEOUT
