...
static pcmk__cluster_option_t fence_attrs[] = {
    /* name, old name, type, allowed values,
     * default value, validator,
     * flags,
     * short description,
...
/*!
 * \internal
 * \brief Output fence device attribute metadata as OCF-like XML
 *
 * \param[in,out] out         Output object
 * \param[in]     name        Fake resource agent name for the option list
 * \param[in]     desc_short  Short description of the option list
 ...
 */
int
pcmk__output_fence_attrs(pcmk__output_t *out, const char *name,
                         const char *desc_short, const char *desc_long,
                         bool all)
{
    return out->message(out, "option-list", name, desc_short, desc_long,
                        (uint32_t) pcmk__opt_none, fence_attrs, all);
}
                      ...
                      enum pcmk__opt_flags filter)
{
    ...
    if (filter == pcmk__opt_fencer) {
        pcmk__output_fence_attrs(tmp_out, name, desc_short, desc_long, true);
    } else {
