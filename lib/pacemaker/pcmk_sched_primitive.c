                       ...
                       bool stop_if_fail)
{
    ...
    if (rsc->next_role == pcmk_role_stopped) {
        pcmk__rsc_trace(rsc,
                        ...
                        rsc->id);
        resource_location(rsc, NULL, -INFINITY, PCMK__META_TARGET_ROLE,
                          rsc->cluster);
    ...
    if (!pcmk_is_set(rsc->flags, pcmk_rsc_managed)) {
        ...
        if (stop_if_fail) {
            pcmk__rsc_debug(rsc,
                            "Forcing %s to stop: " PCMK__OPT_STOP_ALL_RESOURCES,
                            rsc->id);
        }
...
pcmk__primitive_create_actions(pcmk_resource_t *rsc)
{
    ...
    if ((rsc->partial_migration_source != NULL)
         ...
         * PCMK__OPT_STONITH_ENABLED is false). We can start such resources
...
ban_if_not_locked(gpointer data, gpointer user_data)
{
    ...
    if (strcmp(node->details->uname, rsc->lock_node->details->uname) != 0) {
        resource_location(rsc, node, -CRM_SCORE_INFINITY,
                          PCMK__OPT_SHUTDOWN_LOCK, rsc->cluster);
    }
}
