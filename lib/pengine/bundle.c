...
pe__bundle_needs_remote_name(pcmk_resource_t *rsc)
{
    ...
    value = g_hash_table_lookup(params, PCMK__REMOTE_RA_ADDR);
...
pe__unpack_bundle(pcmk_resource_t *rsc, pcmk_scheduler_t *scheduler)
{
    ...
    if (xml_obj && valid_network(bundle_data)) {
        ...
        crm_create_nvpair_xml(xml_set, NULL,
                              PCMK__META_ORDERED, XML_BOOLEAN_TRUE);

        value = pcmk__itoa(bundle_data->nreplicas);
        crm_create_nvpair_xml(xml_set, NULL, PCMK__META_CLONE_MAX, value);
        free(value);

        value = pcmk__itoa(bundle_data->nreplicas_per_host);
        crm_create_nvpair_xml(xml_set, NULL, PCMK__META_CLONE_NODE_MAX, value);
        free(value);

        crm_create_nvpair_xml(xml_set, NULL, PCMK__META_GLOBALLY_UNIQUE,
                              pcmk__btoa(bundle_data->nreplicas_per_host > 1));

        if (bundle_data->promoted_max) {
            crm_create_nvpair_xml(xml_set, NULL,
                                  PCMK__META_PROMOTABLE, XML_BOOLEAN_TRUE);
    ...
    if(xml_resource) {
        ...
        if (bundle_data->attribute_target) {
            g_hash_table_replace(rsc->meta,
                                 strdup(PCMK__META_CONTAINER_ATTR_TARGET),
