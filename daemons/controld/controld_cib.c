 ...
 * (name 2x, (seconds_since_epoch - PCMK__OPT_SHUTDOWN_LOCK_LIMIT) 1x)
 */
#define XPATH_NODE_LRM_UNLOCKED XPATH_NODE_STATE "//" XML_LRM_TAG_RSC_OP    \
                                "|" XPATH_NODE_STATE                        \
                                "//" XML_LRM_TAG_RESOURCE                   \
                                "[not(@" PCMK__OPT_SHUTDOWN_LOCK ") "       \
                                    "or " PCMK__OPT_SHUTDOWN_LOCK "<%lld]"
 ...
 * (name 2x, (seconds_since_epoch - PCMK__OPT_SHUTDOWN_LOCK_LIMIT) 1x, name 1x)
 */
...
#define XPATH_RESOURCE_HISTORY "//" XML_CIB_TAG_STATE                       \
                               ...
                               "/" XML_LRM_TAG_RESOURCE                     \
                               "[@" XML_ATTR_ID "='%s']"
// @TODO could add "and @PCMK__OPT_SHUTDOWN_LOCK" to limit to locks
                                 ...
                                 lrmd_event_data_t *op, time_t lock_time)
{
    ...
    if (lock_time != 0) {
        ...
        crm_xml_add_ll(xml, PCMK__OPT_SHUTDOWN_LOCK, (long long) lock_time);
    }
    if (op->params != NULL) {
