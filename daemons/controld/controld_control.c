...
config_query_callback(xmlNode * msg, int call_id, int rc, xmlNode * output, void *user_data)
{
    ...
    value = g_hash_table_lookup(config_hash, PCMK__OPT_NO_QUORUM_POLICY);
    if (pcmk__str_eq(value, "suicide", pcmk__str_casei) && pcmk__locate_sbd()) {
        controld_set_global_flags(controld_no_quorum_suicide);
    }

    value = g_hash_table_lookup(config_hash, PCMK__OPT_SHUTDOWN_LOCK);
    ...
    value = g_hash_table_lookup(config_hash, PCMK__OPT_NODE_PENDING_TIMEOUT);
    pcmk__parse_interval_spec(value, &controld_globals.node_pending_timeout);
    controld_globals.node_pending_timeout /= 1000;

    value = g_hash_table_lookup(config_hash, PCMK__OPT_CLUSTER_NAME);
